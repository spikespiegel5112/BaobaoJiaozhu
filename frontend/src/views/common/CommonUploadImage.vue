  ...
  <div class="">
    <div class="common_imguploadpreview_wrapper">
      ...
      <div v-if="innerFileList.length > 0">
        ...
        >
          <img
            v-if="checkFileType(item.name || item.url) === 'image'"
            :src="$checkOSS(item.name || item.url, '-style_100x100')"
            class="avatar"
          />
    ...
    >
      ...
      <div v-if="fileType !== ''" slot="tip" class="el-upload__tip">
        只能上传{{
          fileTypeDictionary
            .filter(item => item.name === this.fileType)[0]
            .suffixList.join('、')
        }}文件，且不超过10MB
      </div>
    </el-upload>
...
export default {
  ...
  methods: {
      ...
      if (value.length > 0 && value !== '') {
        value.forEach((item, index) => {
          this.$set(this.innerFileList, index, {
            name: index,
            url: item,
            type: this.fileTypeDictionary.filter(
              item2 => item2 === this.checkFileType(item)
            ).name
          });
        });
      }
    ...
    updateUrlList() {
      if (this.multiple) {
        this.$emit(
          'update:returnUrlList',
          this.innerFileList.map(item => {
            return item.url;
          })
        );
      } else {
    ...
    handleBeforeUpload(file) {
      ...
      if (this.fileType === '' && this.checkFileType(file.name) === '') {
        ...
        this.fileType !== '' &&
        this.checkFileType(file.name) !== this.fileType
      ) {
        this.$message({
          message:
            '文件必须为' +
            this.fileTypeDictionary
              .filter(item => item.name === this.fileType)[0]
              .suffixList.join('、') +
            '类型文件',
          type: 'error'
    ...
    checkFileType(url) {
      ...
      if (url === null || url.lastIndexOf('.') === -1) {
        ...
        this.fileTypeDictionary.forEach((item1, index1) => {
          ...
          if (
            !fileTypeMatchFlag &&
            index1 === this.fileTypeDictionary.length - 1
          ) {
            resultFileType = 'file';
          }
        });
